...
func TestNewHeader(t *testing.T) {
	...
	mh3Mock := &messageHeaderV2{
		messageHeader: messageHeader{
			...
			authenticatedData:     AAD.NewAADWithEncryptionContext(map[string]string{"test": "testing"}),
			encryptedDataKeyCount: 1,
			encryptedDataKeys:     []format.MessageEDK{edk1Mock},
			contentType:           suite.FramedContent,
			frameLength:           1024,
...
func Test_deserializeHeader(t *testing.T) {
	...
	mh1Mock := &messageHeaderV2{
		messageHeader: messageHeader{
			...
			messageID:             messageID,
			aadLen:                188,
			authenticatedData:     AAD.NewAADWithEncryptionContext(map[string]string{"aws-crypto-public-key": "AyeV6x6xnVRv18vTtytzSID5DqpH6ejeNmGbU93PC/quQUnai2AgzTNLzJQglsZF2A==", "keyId": "4a5c8ebf-f7d0-4d09-88c3-5edb48539163", "orgId": "org-uuid-test", "someId": "someId-uuid-test"}),
